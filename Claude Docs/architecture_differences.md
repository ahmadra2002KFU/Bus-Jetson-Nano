...
### 6. Why Threshold-Based Detection Instead of Snort

*   **Initial Plan:** Use Snort (a signature-based Network Intrusion Detection System) running on the Jetson Nano to inspect live network traffic and detect attacks.
*   **The Flaw:** Snort is a real-world tool that inspects real packets on a real network interface (NIC or pcap). Inside ns-3, packets are C++ objects in simulated memory and never touch a physical network interface. There is no way to feed ns-3 packets to Snort without using `RealtimeSimulator` + `TapBridge` to bridge simulated traffic to the host OS network stack. This brings back the real-time vs. virtual-time synchronization problem described in section 1. Additionally, our DDoS attack is a volumetric UDP flood with no distinctive payload signature for Snort to match. Snort would need a custom rate-based rule, which is functionally identical to what we implemented inside the simulator.
*   **Final Plan:** Detection is implemented directly inside the ns-3 simulation using two threshold-based voting systems:
    1. **DDoS Detection** (`CheckDDoS`): Monitors interval traffic rate, packet loss rate, and maximum delay from FlowMonitor every 5 seconds. Uses 2-of-3 voting (any 2 conditions exceeding their thresholds triggers detection).
    2. **GPS Spoofing Detection** (`GpsDetectorApp`): Validates incoming GPS telemetry packets for speed anomaly, position jump, corridor violation, and source IP mismatch. Uses 2-of-4 voting. Also validates a GPS1 payload magic header to ignore non-GPS traffic.
*   **Future Integration:** If the project advances to the hardware-in-the-loop phase (Jetson Nano), Snort could be deployed on the Jetson to inspect real traffic bridged from ns-3 via TapBridge. The threshold-based detection developed here serves as the simulation-phase equivalent and validates the detection logic before hardware deployment.